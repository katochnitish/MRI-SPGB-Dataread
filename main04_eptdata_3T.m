...
datapath   = 'MRDataRaw/Siemens_3T/';
%% Path for twix file format
[filename, dirpath] = uigetfile({'*.*'},'File Selector');
...
%% Scan settings
settings.imSize             = 128;
settings.sequence           = 2; %1 C-SE %2 for TSE %3 for MSME % 4 for bSSFP
...
settings.reconstruct_sigmaH = 'CREPT'; %HEPT %CREPT %SHEPT
%% ===================================================================== %%
[opt_mr,opt_phase,EPTrecon] = SiemensPipeline(filename,filepath,settings);
...
if settings.display
    ...
    figure;montage(EPTrecon.RawData.mag(:,:,sl_tra,:)*1e5,'DisplayRange',...
        [0 100]);colormap gray;axis off image;
    ...
    opt_mr_disp = reshape(EPTrecon.opt_mr*1e5,[EPTrecon.parameters.nPE,...
        EPTrecon.parameters.nRO,1,EPTrecon.parameters.nSlc]);
    figure;montage(opt_mr_disp,'DisplayRange',[0 100]);colormap gray;...
        axis off image;
else
end
